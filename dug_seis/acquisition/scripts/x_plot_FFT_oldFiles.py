...
# Path of asdf files
asdf_folder = Path('/data/testData/sine20kHzTrig/')
asdf_folder = Path('/data/testData/noise_asdf_from_old_spectrum_cards/')
...
for index, file_name in enumerate(asdf_list):
    ...
    start_time = asdf_1.waveforms.XB_032.raw_recording[0].stats.starttime
    end_time = asdf_1.waveforms.XB_032.raw_recording[0].stats.endtime

    for chan in range(32, 33): # 33
        stream_1 += asdf_1.get_waveforms(network='XB', station='032',
                                         location=str(0).zfill(2), channel="001",
