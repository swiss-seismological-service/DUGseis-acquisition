...
for index, file_name in enumerate(asdf_list):
    ...
    start_time = asdf_1.waveforms.XB_2.raw_recording[0].stats.starttime
    daq_nr = '02'
    end_time = asdf_1.waveforms.XB_2.raw_recording[0].stats.endtime
    file_name_file = start_time.strftime('%Y_%m_%dT%H_%M_%S_%f') + '__' + \
                    end_time.strftime('%Y_%m_%dT%H_%M_%S_%f') + '__' + daq_nr + '.h5'
    npts = asdf_1.waveforms.XB_2.raw_recording[0].stats.npts
    asdf_1 = None
